...
<div class="card">
    <h5 class="card-header">Edit Selected Testimony</h5>
    <div class="card-body">
      <form method="post" action="{{route('testimony.update',$testimonials->id)}}">
        @csrf 
        @method('PATCH')
        <div class="form-group">
          <label for="class" class="col-form-label">Language <span class="text-danger">*</span></label>
          <select name="class" class="form-control">
            <option value="eng" {{(($testimonials->class=='eng') ? 'selected' : '')}}>English</option>
            <option value="amh" {{(($testimonials->class=='amh') ? 'selected' : '')}}>Amharic</option>
          </select>
          @error('class')
