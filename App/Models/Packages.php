...
class Packages extends Model
{
     ...
     protected $fillable=['class','title','slug','summary','photo','status','is_parent','parent_id','added_by','is_featured'];
   
    public function parent_info(){
        return $this->hasOne('App\Models\Category','id','parent_id');
    }
    public static function getAllPackage(){
        return  Packages::orderBy('id','DESC')->with('parent_info')->paginate(10);
    }
    ...
    public static function countActivepackages(){
        $data=Packages::where('status','active')->count();
        if($data){
            return $data;
        }
        return 0;
